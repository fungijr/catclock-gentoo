...
static void Tick(XtPointer client_data, XtIntervalId *id)
{
    Widget w = (Widget) client_data;  // Cast the client_data back to a Widget pointer
    ...
    Boolean    *continueToDispatch;
{
    *continueToDispatch = True;
    ...
    if (event->type == MapNotify) {
        iconified = False;
        XSync(dpy, False);
    XtIntervalId interval_id = 0;  // Or a valid value
    Tick(XtWindowToWidget(dpy, clockWindow), &interval_id);
    }
